...
hom.addEventListener('mouseover',function(){
    ...
    this.setAttribute('src','resources/homebtnh.png');

    })
hom.addEventListener('mouseout',function(){
    // this.setAttribute('style','color : #4A4A4A');
    this.setAttribute('src','resources/homebtn.png');
...
const curfl = document.querySelector('.cursor-follower');
        ...
        document.addEventListener('mousemove', e => {
            if(bigcur==false){
                if(e.pageY>2620)
                {
                    cursor.setAttribute("style", "top: "+(e.pageY - 5)+"px; left: "+(e.pageX - 5)+"px; background-color: red; opacity:0.8;");
                    curfl.setAttribute("style", "top: "+(e.pageY - 10)+"px; left: "+(e.pageX - 10)+"px; background-color: red;  ");
